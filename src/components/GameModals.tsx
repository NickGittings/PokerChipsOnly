...
export function StreetModal({ snapshot, send, connected, dealer = false }: Props) {
  ...
  if (!open) return null;
  const buttonPlayer = game.players.find(p => p.id === dealerId(game));
  const dealerName = buttonPlayer?.connected ? buttonPlayer.name : 'the board';
  const cards = game.awaitingDeal ? 'hole cards' : game.pendingStreet;
  const total = game.players.reduce((sum, p) => sum + p.committedThisHand, 0);
  return <div className="game-overlay" ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="street-title"><section className="street-modal"><span className="eyebrow">A moment for the dealer</span><div className="street-symbol" aria-hidden="true">♠</div><h1 id="street-title">{dealer ? <>Deal the{' '}<br /><em>{cards}</em></> : <>Waiting for {dealerName}{' '}<br /><em>to deal</em></>}</h1><p>{dealer ? game.awaitingDeal ? 'Deal two cards face down to each player, starting left of the button.' : game.pendingStreet === 'flop' ? 'Burn one card. Deal three cards face up.' : 'Burn one card. Deal one card face up.' : `The ${cards} are next. ${dealerName === 'the board' ? 'The board' : dealerName} will confirm when the cards are dealt.`}</p><div className="street-pot"><ChipStack amount={total} denominations={game.config.denominations} /><strong>{money(total)} in the pot</strong><span>{game.players.filter(p => p.status === 'active' || p.status === 'all-in').length} players still in</span></div>{dealer ? <button className="game-button primary wide" disabled={!connected} onClick={() => send({ type: 'dealerConfirm', revision: game.revision })}>Cards dealt <span>→</span></button> : <div className="waiting-note"><span className="pulse-dot" /> Betting {game.awaitingDeal ? 'starts' : 'resumes'} when the dealer is ready</div>}</section></div>;
}
