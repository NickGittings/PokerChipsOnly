...
export function handsToNextLevel(g: GameState): number { return g.pendingLevel !== g.level ? 1 : Math.max(1, g.config.levelHands - Math.max(1, g.hand) + g.levelStartHand + 1); }
export function levelNotice(g: GameState): string | null {
  return g.pendingLevel === g.level ? g.config.blindPace === 'hands' ? `Next level · in ${handsToNextLevel(g)} ${handsToNextLevel(g) === 1 ? 'hand' : 'hands'}` : null : `Blinds ${g.pendingLevel > g.level ? 'up' : 'down'} next hand · level ${g.pendingLevel}`;
}
export function validateConfig(c: Config): string[] {
  ...
  if (!['time', 'hands'].includes(c.blindPace)) errors.push('Choose blinds by time or hands.');
  if (!Number.isInteger(c.levelHands) || c.levelHands < 1 || c.levelHands > 100) errors.push('Hands per level must be a whole number from 1–100.');
  if (c.blindPace === 'time' && (!Number.isFinite(c.levelMinutes) || c.levelMinutes < 1 || c.levelMinutes > 180)) errors.push('Level length must be 1–180 minutes.');
