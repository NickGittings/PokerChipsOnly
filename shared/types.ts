...
export type ClientMsg = { type: 'join'; token: string; name?: string; board?: boolean } | { type: 'claimSeat'; seat: number; name: string } | { type: 'leaveSeat' } | { type: 'rebuy' } | { type: 'reclaimSeat'; playerId: string } | { type: 'setJoinUrl'; url: string } | { type: 'lateBuyIn'; name: string; seat: number } | { type: 'startTournament'; config: Config } | { type: 'action'; action: Action; revision: number } | { type: 'dealerConfirm' | 'nextHand' | 'undo' | 'pauseClock'; revision: number } | { type: 'awardPot'; potIndex: number; winnerIds: string[]; revision: number } | { type: 'hostAdjust'; playerId: string; delta: number; revision: number } | { type: 'colorUp'; revision: number };
