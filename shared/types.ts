...
export type ClientMsg = { type: 'join'; token: string; name?: string; board?: boolean } | { type: 'claimSeat'; seat: number; name: string } | { type: 'leaveSeat' } | { type: 'startTournament'; config: Config } | { type: 'action'; action: Action; revision: number } | { type: 'dealerConfirm' | 'nextHand' | 'undo' | 'pauseClock'; revision: number } | { type: 'awardPot'; potIndex: number; winnerIds: string[]; revision: number } | { type: 'hostAdjust'; playerId: string; delta: number; revision: number } | { type: 'colorUp'; revision: number };
export interface Snapshot { game: GameState; you: { id: string; host: boolean; dealer: boolean; legal: LegalActions | null }; joinUrl: string; canUndo: boolean; serverTime: number }
